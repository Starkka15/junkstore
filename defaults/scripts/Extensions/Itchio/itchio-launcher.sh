...
echo "dbfile: ${DBFILE}"
SETTINGS=$($ITCHIOCONF --get-env-settings $ID --dbfile $DBFILE --platform Proton --fork "" --version "" --dbfile $DBFILE)
echo "${SETTINGS}"
eval "${SETTINGS}"
...
if [[ "${ADVANCED_IGNORE_ITCHIO_ARGS}" == "true" ]]; then
...
GAME_PATH=$($ITCHIOCONF --get-game-dir "$ID" --dbfile $DBFILE)
popd
...
if [[ "${ITCHIO_ENABLE_UMU_FIXES}" != "false" ]]; then
    export STORE="itchio"
    UMU_ID=$($ITCHIOCONF --get-umu-id "$ID" --dbfile $DBFILE)
