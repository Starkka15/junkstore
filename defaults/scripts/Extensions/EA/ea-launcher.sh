...
echo "dbfile: ${DBFILE}"
SETTINGS=$($EACONF --get-env-settings $ID --dbfile $DBFILE --platform Proton --fork "" --version "" --dbfile $DBFILE)
echo "${SETTINGS}"
eval "${SETTINGS}"
...
GAME_PATH=$($EACONF --get-game-dir "$ID" --dbfile $DBFILE)
popd
...
if [[ "${EA_ENABLE_UMU_FIXES}" != "false" ]]; then
    export STORE="ea"
    UMU_ID=$($EACONF --get-umu-id "$ID" --dbfile $DBFILE)
